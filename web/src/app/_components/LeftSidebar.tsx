...
export default function LeftSidebar({ isSidebarOpen, toggleSidebar, user, logout }: LeftSidebarProps) {
  ...
  const displayedNavItems = isSidebarOpen
    ...
    : sidebarNavItems;
     ...
     const firstLetter = user?.profile?.firstName?.charAt(0)?.toUpperCase() || 'U';
      ...
      <div className="p-4">
        <div className='bg-white cursor-pointe rounded p-3'>
          <div className='flex items-center justify-between  '>
            ...
            <BiPlus className='text-2xl border-gray-600 rounded-full border-2 p-1 cursor-pointer' />
          </div>
          {user && (
