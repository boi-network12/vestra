...
const Following: FC<FollowingProps> = ({
  ...
  const handleUnfollow = useMemo(
    () =>
      debounce(async (userId: string, isMutual: boolean) => {
        if (isMutual) {
          const user = following.find((u) => u._id === userId);
          ...
          if (result.success) {
            ...
            await getFollowingWithDetails();
          } else {
            showAlert(result.message, "error");
          }
        }
      }, 300),
    [debounce, unfollowUser, getFollowingWithDetails, showAlert, following]
  );
  ...
  const confirmUnfollow = useMemo(
    ...
    [debounce, unfollowUser, getFollowingWithDetails, showAlert]
  );
